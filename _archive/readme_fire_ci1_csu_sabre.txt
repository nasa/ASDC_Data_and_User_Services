FIRE Cirrus 1 Colorado State University 
Sabreliner Meteorological and Radiometric Data Set
...
To build the sample read program from the C source files apply the `make' 
command to the make file (which defines the C source files listed in Section 2.0.)  
...
is the only one needed for the FIRE_CI1_CSU_SABRE data set. 
...
Makefile.fci1 assumes the library file, 'fire_lib.c', and the include file, 'fire_sdf.h', 
...
argument is the data filename.   For example, type:
	...
	% fsdf ci1_csu_sabre.hdr ci1_csu_sabre.toc ci1_sabr_861013_1102.001 
...
5.1.1  Meteorological and Radiometric Data from the NCAR Sabreliner

FILE NAMING CONVENTION:

            ci1_csu_sabre.hdr        (header file)

            ci1_csu_sabre.toc        (VTOC file)

            ci1_csu_sabre.txt        (ASCII representation of the first data file, .004)

            ci1_csu_sabre_861013     (granule name)

            ci1_sabr_861013_1102.001 (first binary data file in the granule)

There are 12 granules in this data set.

VARIABLES:

All records and parameters within this data set have been defined, including their 
minimum and maximum values, in the header file, ci1_csu_sabre.hdr.

NOTES ON DATA:

After testing this data set, we have found that the southern and northern latitude values are 
written in reverse order.  This information is written in the VTOC file as well as in each of 
the data record files. The data producer recommends that you be made aware of this 
finding.

The fill value number is the maximum 4-byte integer word size of 2147483647.

There are two full records of (176*2) ASCII header information in each data record file.
...
Phone:   970-491-8594 
FAX:     970-491-8449  
e-mail:  scox@lamar.colostate.edu 
