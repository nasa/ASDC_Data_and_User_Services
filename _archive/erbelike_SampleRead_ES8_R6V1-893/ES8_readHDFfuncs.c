...
---------------------------------------------------------------
*/
char	*list_sds(char *filename, int32 *field_quantity)
{
	...
	iret=SDfileinfo(sd_id,&ndatasets,&nglobal_attr);

	*field_quantity=ndatasets;
	for (i=0; i<ndatasets; i++)
	{
		sds_id=SDselect(sd_id,i);
		iret=SDgetinfo(sds_id,sds_name,&rank,dimsizes,&number_type,
                                 &nattrs);

		if (i==0) 
		{
			strcpy(result,sds_name);
		}
		else
		{
			strcat(result,",");
			strcat(result,sds_name);
		}
	...
	iret=SDend(sd_id);
	the_result = malloc(sizeof(result)+1);
	if(the_result == NULL)
	{
		fprintf(stderr, "Unable to allocate memory in list_sds.");
		exit(1);
	}

	strcpy(the_result, result);
   return(the_result);
}
/*
---------------------------------------------------------------
...
---------------------------------------------------------------
*/
char	*list_vdata(char *filename, int32 *field_quantity)
{
	...
	int		continue_flag;
   	...
   	int32		previous_vref;
   	char		result[MAX_LIST];
	char		vdata_name[MAX_VDATA_NAMES];
	char		*the_result;
...
/*    fprintf(stdout,"current_vid=%i   vdata name=%s\n",current_vid,vdata_name);   */
					...
					{
						if(number_of_vdatas > 0)
                                                   strcat(result,",");
						strcat(result,vdata_name);
						number_of_vdatas++;
					}
	...
	*field_quantity=number_of_vdatas;
	...
	if(the_result == NULL)
	{
		fprintf(stderr, "Unable to allocate memory in list_vdata.");
		exit(1);
	}
                          ...
                          int32 *field_quantity)
{
	...
	/*
   	...
   	----------------------------------------
   	*/
   	char		dummy_names[MAX_FNAMES];
   	int32		current_vid;
   	int32		current_vref;
   	char		field_names[MAX_FNAMES];
   	int32        	file_id;
   	int32		number_of_fields;
   	char		result[MAX_VDATA_FIELDS];
	...
	{
		Vstart(file_id);
		/* current_vref=VSgetid(file_id, -1); */
...
/*		if (current_vref!=-1)  */
		if (current_vref)
		{
			...
			*field_quantity=number_of_fields;
                   /*   fprintf(stdout,
                           "\tNumber of Fields = \t%i\n",number_of_fields);  */
			set_field_names(field_names,
                                        number_of_fields,current_vid); 
			/*
			...
			*/
			...
			Vend(file_id);
   			Hclose(file_id);
			strcpy(result,field_names);	
			the_result=malloc(strlen(result)+1);
			strcpy(the_result, result);
		   }
                   else
                   {
...
void	set_field_names(char *the_names, int32 count, int32 vid)
{
	...
	for (i=0; i<count; i++) 
	{
		temp_name=VFfieldname(vid,i);
		strcat(the_names,temp_name);
		if (i!=count-1)
		{
			strcat(the_names,",");
		}
...
---------------------------------------------------------------
*/
void     get_sds_dims_by_index(char *filename, int32 index, char *sds_name,
                               int32 *output_rank, int32 *output_dimensions,
                                                  int32 *output_number_type)
{
...

{
        ...
        */
        ...
        if(sds_id == -1)
        {
           fprintf(stderr,"Could not get sds_id for sds_index %i.\n", index);
           status = SDend(sd_id);
           result = -1;
           return result;
        }
        ...
        if(status == -1)
        {
           fprintf(stderr, "ERROR for reading sds_index %i.\n", index);
           result = -1;
        }
        else
        {
                       ...
                       int32 *output_types, int32 *output_order)
{
	...
	*/
	...
	/*	                current_vfield_index=vfield_indices[i];    */
                ...
                
{
	...
	char		*ptr_temp;
   	...
   	int32		status;
	int32		this_index;
	int32		the_index[MAX_VDATA_FIELDS];
		...
		if (current_vref)
		{
		   ...
		   if (current_vid!=-1)
		   {
			if (field_count!=-1)
			{
			   ...
			   if (status==-1)
	  		   {
			   	fprintf(stdout,"\tEven VSseek fails!\n");
				exit(-1);
			   }
					 	...
					 	{	
						   this_index = the_index[k];
							...
							case DFNT_CHAR8:
								...
								/* memcpy(output_data[k], buffer_ptr, data_size); */
...
void  set_vdata_data(int32 num_fields, int32 *number_type, int32 *order,
                     int32 n_records, void *ptr[])
{
  ...
  for (i=0; i<num_fields; i++)
  {
     ...
     switch(number_type[i])
     {
        case  3:
               {
	          stor_size = data_size * sizeof(uchar8);
                  if ( order[i] == 1 ) ptr[i] = (uchar8 *) malloc(stor_size);
                  else ptr[i] = (uchar8 *) calloc(n_records,stor_size);
		  break;
               }
        case  4:
               {
                  stor_size = data_size * sizeof(char8);
                  if ( order[i] == 1 ) ptr[i] = (char8 *) malloc(stor_size);
                  else ptr[i] = (char8 *) calloc(n_records,stor_size);
		  break;
               }
        case  5:
               {
                  stor_size = data_size * sizeof(float32);
                  if ( order[i] == 1 ) ptr[i] = (float32 *) malloc(stor_size);
                  else ptr[i] = (float32 *) calloc(n_records,stor_size);
		  break;
               }
	case  6:
               {
                  stor_size = data_size * sizeof(float64);
                  if ( order[i] == 1 ) ptr[i] = (float64 *) malloc(stor_size);
                  else ptr[i] = (float64 *) calloc(n_records,stor_size);
		  break;
               }
	case 20:
               {
                  stor_size = data_size * sizeof(int8);
                  if ( order[i] == 1 ) ptr[i] = (int8 *) malloc(stor_size);
                  else ptr[i] = (int8 *) calloc(n_records,stor_size);
		  break;
               }
	case 21:
               {
                  stor_size = data_size * sizeof(uint8);
                  if ( order[i] == 1 ) ptr[i] = (uint8 *) malloc(stor_size);
                  else ptr[i] = (uint8 *) calloc(n_records,stor_size);
		  break;
               }
	case 22:
               {
                  stor_size = data_size * sizeof(int16);
                  if ( order[i] == 1 ) ptr[i] = (int16 *) malloc(stor_size);
                  else ptr[i] = (int16 *) calloc(n_records,stor_size);
		  break;
               }
	case 23:
               {
                  stor_size = data_size * sizeof(uint16);
                  if ( order[i] == 1 ) ptr[i] = (uint16 *) malloc(stor_size);
                  else ptr[i] = (uint16 *) calloc(n_records,stor_size);
		  break;
               }
	case 24:
               {
                  stor_size = data_size * sizeof(int32);
                  if ( order[i] == 1 ) ptr[i] = (int32 *) malloc(stor_size);
                  else ptr[i] = (int32 *) calloc(n_records,stor_size);
		  break;
               }
	case 25:
	       {
                  stor_size = data_size * sizeof(uint32);
                  if ( order[i] == 1 ) ptr[i] = (uint32 *) malloc(stor_size);
                  else ptr[i] = (uint32 *) calloc(n_records,stor_size);
		  break;
	       }
     /*
        case 26:
	       {
                  stor_size = data_size * sizeof(int64);
                  if ( order[i] == 1 ) ptr[i] = (int64 *) malloc(stor_size);
                  else ptr[i] = (int64 *) calloc(n_records,stor_size);
		  break;
	       }
	case 27:
	       {
                  stor_size = data_size * sizeof(uint64);
                  if ( order[i] == 1 ) ptr[i] = (uint64 *) malloc(stor_size);
                  else ptr[i] = (uint64 *) calloc(n_records,stor_size);
		  break;
	       }
     */
	default:
...
void free_vdata(int32 n_fields, void *vdata[])
{
...
-----------------------------------------------------------
*/
int32 *Get_num_records (char *HDF_file)
{
  int32 file_id, status, num_records;
  int32 sd_id, sds_index, sds_id, rank, nattrs, num_type;
  int32 dimsize[MAX_SDS_DIMS];
  char dimname[MAX_NC_NAME];

  file_id = Hopen(HDF_file, DFACC_RDONLY, 0);

  if (file_id != -1)
    {
      sd_id = SDstart(HDF_file, DFACC_RDONLY);
      sds_index = SDnametoindex(sd_id, "Colatitude of CERES FOV at TOA");
      sds_id = SDselect(sd_id, sds_index);

      status = SDgetinfo(sds_id, dimname, &rank, dimsize, &num_type, &nattrs);

      if (status == -1)
	{
	  printf("Error reading sds\n");
	  status = SDendaccess(sds_id);
	  status = SDend(sd_id);
	  return;
	}

      num_records = dimsize[0];
