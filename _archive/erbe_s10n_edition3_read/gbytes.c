 ...
 *
 * p          pointer to packed array
 * u          pointer to unpacked array
 * q >= 0     number of bits to be skipped preceeding first byte in p
 * 0 < b < s  word byte size
...
long *p, *u, *q, *b;
{
   ...
   /* *t = (*t & ~(MASK << (SWORD - jb) >> jq)) + */
   /* ((~(MASK << jb) & *u) << SWORD-(jb+jq)); */
   *t = (*t & ~(MASK << (SWORD - jb) >> jq)) +
        (((unsigned) *u) << (SWORD - jb) >> jq);
}
