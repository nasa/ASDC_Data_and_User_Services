...
is the only one needed for the FIRE_CI1_TOVS data set. 
...
argument is the data filename.   For example, type:
	...
	% fsdf ci1_tovs.hdr ci1_tovs.toc ci1_tovs_861014_1102.001 
...
5.1.1  TOVS Data Files 

FILE NAMING CONVENTION:

            ci1_tovs.hdr        (header file)

            ci1_tovs.toc        (VTOC file)

            ci1_tovs.txt        (ASCII representation of the first data file, .004)

            ci1_tovs_861013     (granule name)

            ci1_tovs_861014_1102.001 (first binary data file in the granule)

This data set contains 18 granules.

VARIABLES:

All records and parameters within this data set have been defined, including their 
minimum and maximum values, in the header file, ci1_tovs.hdr.

NOTES ON DATA:

Modifications were made to the header file at the data producer's request.  (1) The 
record name was missing from the record definition. This has been included.  (2) The 
word Description was misspelled in the section called Instrument Description - the 
letter p was missing.  (3) The parameter Time had a minimum value of 1. This value has 
been changed to 0.  (4) The parameter HHMM has been changed to HHMMSS to reflect the 
data.  The minimum value for this parameter has been changed from 1 to 0.  The maximum
value for this parameter has been changed to 240000.
