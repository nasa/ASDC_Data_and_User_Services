FIRE Cirrus 1 NASA ER-2 LIDAR Data Set 
...
is the only one needed for the FIRE_CI1_ER2_LIDAR data set. 
...
argument is the data filename.   For example, type:
	...
	% fsdf ci1_er2_lidar.hdr ci1_er2_lidar.toc ci1_spin_861013_1102.001 
...
5.1.1  NASA ER-2 LIDAR Data Files 

FILE NAMING CONVENTION:

            ci1_er2_lidar.hdr        (header file)

            ci1_er2_lidar.toc        (VTOC file)

            ci1_er2_lidar.txt        (ASCII representation of the first data file, .004)

            ci1_er2_lidar_861013     (granule name)

            ci1_spin_861013_1102.001 (first binary data file in the granule)
...
minimum and maximum values, in the header file, ci1_er2_lidar.hdr.

NOTES ON DATA:

The header file states that there are 80 bytes in each data record.  The last four 
bytes in each record are padded with zeroes.

The undetected data value has been flagged with a -9.9.

An integer value appears at the end of the file.  The data producer is not sure why 
this same integer appears in several files.  The data producer requests that this 
value be ignored.   This integer value is 538976288.  This sample read program has 
been written to ignore these values.

The fill value number for this data set is the maximum 4-byte integer word size of 
2147483647.

The ASCII header record contains 160 bytes; however, the spacing is not the same as what 
has been defined in the SDF document.

The GMT date within the ASCII header record of each data file contains blanks instead of 
zeroes.
...
Phone: (301) 286-5578
Fax:   (301) 286-1762
e-mail:   sgdlh@virl.gsfc.nasa.gov
