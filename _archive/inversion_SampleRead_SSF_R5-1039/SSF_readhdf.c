/*
 *****************************************************************************
 *  Program: SSF_readhdf.c
 *  
 *  Purpose: To provide examples of how to read data from the SSF HDF file.
 *  
 ...
 *   HDF_File - SSF HDF file name - modify this if a different SSF HDF file is
 *              being read
 *   sds_name - SDS name - see the Data Products Catalog ("SSF_dpc_r3.ps") or 
 *              the provided dump of the HDF file ("SSF_hdf_5rec_dump") 
 *              for valid SDS names in the SSF product
 ...
 *******************************************************************************
     ...
     HDF_File = "CER_SSF_Terra-FM2-MODIS_Sample_000000.2000100101"; 
*/
/* ===========================================================================
...
============================================================================*/
  ...
  if(argc != 2)
  {
     fprintf ( stderr, "Usage: ssf_rd HDFfilename\n");
     exit(1);
  }
...
*/
     ...
     sds_name = "Surface type percent coverage";
     get_sds_dims(HDF_File,sds_name,&rank,sds_dims,&type);
     sds_rank = rank;
     sds_type = type;
     sds_edge[0] = sds_dims[0]; 
...
/*
 *******************************************************************************
 *  Read the "Surface type percent coverage" SDS into the 1-D "surface_types" 
 *  array. 
 *******************************************************************************
*/
...
*/
     ...
     for (i=0; i<sds_dims[0]; i++)
     {
        ...
        {
           k = i * sds_dims[1] + j;
           printf ("\nSurface type percent coverage(%i) = %i",
                      j+1,surface_types[k]);
 ...
 *******************************************************************************
*/
     sds_name    = "Percentiles of visible optical depth for cloud layer";
     get_sds_dims(HDF_File,sds_name,&rank,sds_dims,&type);
     sds_rank = rank;
     sds_type = type;
     sds_edge[0] = sds_dims[0]; 
...
*/
     ...
     for (i=0; i<sds_dims[0]; i++)
     {
        ...
        fprintf (stdout,"\n\nFootprint %i\n",i+1);
        fprintf (stdout,"\nPercentiles of visible optical depth for cloud layer ");

        for (j=0; j<13; j++)

           { for (k=0; k<2; k++)
                 { if ( k == 0)
                      { fprintf (stdout,"\nPercentile %i for Lower Layer = %f", j+1,
                          *((float32 *)sds_data+index+j*2)); }
                   else 
                      { fprintf (stdout,"\nPercentile %i for Upper Layer = %f", j+1,
                          *((float32 *)sds_data+index+j*2+1)); }
                 }
           }
 ...
 *             (ie., there are 20 values of wavelength) from the "SSF_Header" 
 ...
 *             The two valid Vdatas in the SSF are "SSF_Header" and 
 *             "CERES_metadata".  The variable "num_fields" must be set to 23 
 *             for the "SSF_Header" Vdata and 14 for the "CERES_metadata" Vdata.
...
/*
 *******************************************************************************
 * The Vdatas in the SSF contain only one record each and in these examples, 
