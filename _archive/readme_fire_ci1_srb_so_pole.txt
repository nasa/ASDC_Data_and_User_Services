FIRE Cirrus 1 Surface Radiation Budget (SRB) South Pole Data Set 
...
is the only one needed for the FIRE_CI1_SRB_SO_POLE data set. 
...
the data filename (i.e., datasetname.filenumber)  and the fourth argument is 
the ancillary data file (datasetname.001).   
For example, type:

% fci1_srb ci1_srb_sp.hdr ci1_srb_sp.toc ci1_srb_sp_861025_1031.010 ci1_srb_sp_861025_31.001

The executable 'fci1_srb' will read the binary data files found in each data granule 
and create an output file called datasetname.filenumber.OBS, i.e., ci1_srb_sp_861025_31.010.OBS.  
...
5.1.1  SRB South Pole Data Files 

FILE NAMING CONVENTION:

ci1_srb_sp.hdr              Header file, in CHARACTER format (ASCII).
                            Physical record length is 6400 bytes.
                            Logical record length is 80 bytes.

ci1_srb_sp.toc              Volume Table of Contents (VTOC) in ASCII.
                            Physical record length is 6400 bytes.
...
ci1_srb_sp_861001_05.txt    Test Data in ASCII.  There are 6 test data files each
ci1_srb_sp_861006_10.txt    for a different time period.  Physical record length is 
ci1_srb_sp_861010_15.txt    6400 bytes.  Logical record length is 80 bytes.  Each 
ci1_srb_sp_861015_20.txt    logical record is an expanded "pixel packet", consisting 
ci1_srb_sp_861020_25.txt    of RSIZ bytes of the original file per "Test Data" logical 
ci1_srb_sp_861025_31.txt    record.  Note that this file represents the complete contents 
                            of the first "observation data" file, NOT of the first "data"  
                            file (the ancillary file).
            
ci1_srb_sp_861001_05.001    There are 6 ancillary files for this data set each for
ci1_srb_sp_861006_10.001    a different time period. Ancillary File (Lat/Lon grid) in 
ci1_srb_sp_861010_15.001    scaled INTEGER*2 format.  Record prefix length is RPFX 
ci1_srb_sp_861015_20.001    bytes.  Logical record length is RSIZ bytes.  There are 
ci1_srb_sp_861020_25.001    NREC logical records, or "pixels", per physical record.
ci1_srb_sp_861025_31.001    Physical record length is thus (RPFX + NREC*RSIZ) bytes.
                            Within each logical record ("pixel") of the DATA area,
                            bytes 1 and 2 contain a LATITUDE in I*2 format,
                            bytes 3 and 4 contain a LONGITUDE in I*2 format;
                            currently, bytes 5 through RSIZ of each logical record
                            are unused, and are reserved for possible future use.
                            The LAT/LON quantities are to be used as a lookup table
                            to convert any (pixel, scanline) coordinate pair in each
                            of the data files into a pair of LAT/LON coordinates.
                            First physical record is header record, in ASCII.
 
ci1_srb_sp_860930           Granule name.  Organizational strucuture of data from DAAC.
                            This data set contains 32 granules.

ci1_srb_sp_861013_1102.002  Files no. X through last - Where "X" = (1 + no. of 
