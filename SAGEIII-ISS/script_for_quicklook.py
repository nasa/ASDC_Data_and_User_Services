...
def pull_h2o_no2_o3(parameter, start_date, end_date):
    ...
    for y, m in zip(years, months):
        with open(f"{parameter}_{start_date}_{end_date}/{parameter}_{y}_{m}.jpg", "wb") as images:
            images.write(
                requests.get(
                    f"https://sage.nasa.gov/sageiii-iss/browse_images/quicklook/SAGEWeb/Quicklook/monthly_zonal_mean/{y}/{m}//QL_MONTHLY_ZMEAN_{parameter}_{y}{m}.jpg"
                ).content
            )
            if (
                requests.get(
    ...
    try:
        ...
        frames[0].save(
            f"{parameter}_{start_date}_{end_date}/{parameter}_{start_date}_{end_date}.gif",
            format="GIF",
        ...
        print(
            "If you would like to visit SAGE III-ISS Quicklook Browse Imagery services, please go to the link provided below. \nhttps://sage.nasa.gov/sageiii-iss/browse_images/quicklook/index.php"
        )
...
def pull_aer_atm(parameter, wavelength, start_date, end_date):
    ...
    for y, m in zip(years, months):
        with open(
            f"{parameter}_{wavelength}_{start_date}_{end_date}/{parameter}_{wavelength}_{y}_{m}.jpg",
            "wb",
        ) as images:
            images.write(
                requests.get(
                    f"https://sage.nasa.gov/sageiii-iss/browse_images/quicklook/SAGEWeb/Quicklook/monthly_zonal_mean/{y}/{m}//QL_MONTHLY_ZMEAN_{parameter}{wavelength}_{y}{m}.jpg"
                ).content
            )
            if (
                requests.get(
    ...
    try:
        for list in dir_list:
            ...
            frames.append(new_frame)
        frames[0].save(
            f"{parameter}_{wavelength}_{start_date}_{end_date}/{parameter}_{wavelength}_{start_date}_{end_date}.gif",
            format="GIF",
            append_images=frames,
            save_all=True,
            duration=700,
...
def input_parameter():
    ...
    while not valid_answer:
        ...
        : """)
        if (
            answer_parameter.upper() == "H2O"
            or answer_parameter.upper() == "NO2"
            or answer_parameter.upper() == "O3"
            or answer_parameter.upper() == "AER"
            or answer_parameter.upper() == "ATM"
        ):
            break
        else:
...
def input_wavlength():
    ...
    while not valid_answer:
        ...
        if (
            answer_wavelength == "0384"
            or answer_wavelength == "0449"
            or answer_wavelength == "0520"
            or answer_wavelength == "0602"
            or answer_wavelength == "0676"
            or answer_wavelength == "0756"
            or answer_wavelength == "0869"
            or answer_wavelength == "1021"
            or answer_wavelength == "1544"
        ):
            break
        else:
...
def main():
    ...
    while not valid_answer:
        ...
        if answer_one.upper() == "A":
            ...
            if (
                parameter.upper() == "H2O"
                or parameter.upper() == "NO2"
                or parameter.upper() == "O3"
            ):
