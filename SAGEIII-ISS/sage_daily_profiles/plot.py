...
def plot_data(dates, file_lists, filename_each_day, dict_ss, dict_sr, collection, sage_profile_dir):
    for date, file_list, filename_list, ss_values, sr_values in zip(
        sorted(dates), file_lists, filename_each_day, dict_ss.values(), dict_sr.values()
    ):
        if len(ss_values) > 0 and len(sr_values) > 0:
            plot_two(
                date, file_list, ss_values, sr_values, filename_list, collection, sage_profile_dir
            )
    ...
    for dset in dsets:
        for f, b, r in zip(file_list, range(blues.N), range(reds.N)):
            ...
            with h5py.File(f, mode="r") as f:
                ...
                fig.savefig(
                    f"{sage_profile_dir}/{collection}/{date}/{pngfile}",
                    dpi=500,
                    bbox_inches="tight",
                )

        print(
            f"Done. Plot for {data_title} on {date} located here: ~/sage_daily_profiles/{collection}/{date}"
        )
        plt.close()
...
def plot_one(date, file_list, values, color, title, collection, sage_profile_dir):
    for dset in dsets:
        for c, f in zip(range(color.N), file_list):
            ...
            pngfile = "{0}_{1}.png".format(date, dset)
            fig.savefig(
                f"{sage_profile_dir}/{collection}/{date}/{pngfile}", dpi=500, bbox_inches="tight"
            )
