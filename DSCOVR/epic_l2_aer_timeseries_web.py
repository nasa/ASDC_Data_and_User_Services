...
# and returns arrays of wavelengths, 2D latitudes and longitudes, 3D AOD and its fill value, scalar.   
  try:
    ...
    aod3D = 0.
    fv = 0.
    return wl, lat2D, lon2D, aod3D, fv
...
# print item is a dataset, its dimensions, and fill value
      ...
      if 'FinalAerosolOpticalDepth' in name:
        aod3D = f[name][:]
        fv = item.fillvalue
...
def list_epic_L2_AER(yyyymm):
# function list_epic_L2_AER takes month of interest in the form of an 6-character string yyyymm.
# the function returns a list of all DSCOVR_EPIC_L2_AER files publicly availabe at the ASDC, see variable url below.
...
# if token is outdated, the list will not be created.   
  yyyy = yyyymm[0 : 4]
  mm   = yyyymm[4 : 6]
  url = 'https://asdc.larc.nasa.gov/data/DSCOVR/EPIC/L2_AER_03/'
...
#  sys.exit()
  ...
  ref_lines = [i for i in lines if ('href' in i) and ('DSCOVR_EPIC_L2_AER_03' in i)]
  ...
  for line in ref_lines:
    ...
    if pos1 > 0 and pos2 > pos1 and line[pos2-3 : pos2] =='he5':
      cnt = cnt + 1
...
# a list of EPIC_L2_AER_03 files available for that month at DDD,
# https://asdc.larc.nasa.gov/data/DSCOVR/EPIC/L2_AER_03/yyyy/mm/, is created on the fly.
...
yyyymm = sys.argv[1]
refs = list_epic_L2_AER(yyyymm)

# open output file
fout = open('AOD_output_'+yyyymm, 'w')
fout.write('  timestamp   ')
for i in range(npositions):
  for j in range(3): # this is the cycle by wavelengths
    fout.write(' POI%1i_wl%1i' %(i+1, j+1))
...
for ref in refs:
  ...
  pos = epic_name.index('DSCOVR_EPIC_L2_AER_03') + 22
  ...
  wl, lat2D, lon2D, aod3D, fv = read_epic_l2_aer(epic_name)
  ...
  nwl = len(wl)
  aod3Dgrid = np.empty(ngrid3D)

  for i in range(nwl):

# this is the call of re-gridding routine
#  lat1D, lon1D, aod3Dgrid[:, :, i] = grid_2D(lat2D, lon2D, aod3D[:, :, i], ngrid2D, fv)
    aod3Dgrid[:, :, i] = grid_2D(lat2D, lon2D, aod3D[:, :, i], fv, lat1D, lon1D)
