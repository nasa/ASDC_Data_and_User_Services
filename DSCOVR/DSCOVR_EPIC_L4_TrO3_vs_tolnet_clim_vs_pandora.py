...
# o3_column, tro3_column - total and tropospheric ozone columns, DU 
  ...
  dsets = hdf.datasets()
  print('there are ',len(dsets), 'in ', fname)
  ...
  for dset in dsets:
    ...
    if dset == 'LATITUDE.INSTRUMENT':
      print(dset, data[0])
      lat_int =  data[0] # latitude of TOLNTET instrument, this is used to interpolate EPIC L4 ozone data
    if dset == 'LONGITUDE.INSTRUMENT':
      print(dset, data[0])
      lon_int = data[0] # longitude of TOLNTET instrument, this is used to interpolate EPIC L4 ozone data
...
# this is not recommended as downlading again takes extra time.  
#  os.remove(fname)
